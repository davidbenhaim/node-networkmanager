...
NetworkManager = (function(_super) {
  ...
  function NetworkManager(options) {
    ...
    this.connectionSpy = setInterval(this.check_connection, 2 * 1000);
    this.killing = false;
  ...
  NetworkManager.prototype._connectWPA = function(network) {
    var args, command, d, ondata, timeout, wpa, wps;
    d = Q.defer();
    ...
    args = ["wpa_supplicant", '-d', '-i', this.wireless, '-D', 'wext', '-c', '/etc/wpa_supplicant.conf'];
    wps = spawn("sudo", args);
    timeout = setInterval((function(_this) {
      return function() {
        if (!_this.connected) {
