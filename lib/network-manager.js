var EventEmitter, NetworkManager, Q, exec, util, _,
...
NetworkManager = (function(_super) {
  ...
  NetworkManager.prototype._connectWPA = function(network) {
    var command, d;
    d = Q.defer();
    command = "sudo wpa_passphrase \"" + network.ESSID + "\" " + network.PASSWORD + " > /tmp/wpa-temp.conf && sudo wpa_supplicant -D wext -i " + this.wireless + " -c /tmp/wpa-temp.conf && rm /tmp/wpa-temp.conf";
    exec(command, function(error, stdout, stderr) {
      if (error || stderr) {
        console.log(err);
        console.log(stderr);
